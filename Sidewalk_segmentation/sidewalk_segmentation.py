...
dataset = Dataset(x_train_dir, y_train_dir, classes=['current'], augmentation=get_training_augmentation())
...
dice_loss = sm.losses.DiceLoss(class_weights=np.array([1.])) 
...
MODEL_NAME = 'best_model'+BACKBONE
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Test'], loc='upper left')
plt.savefig('training_results_{}.png'.format(BACKBONE))
...
for i in ids:
    ...
    print('[INFO] single inference took {}'.format(time.time()-start))
